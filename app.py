...
# Always use verified baseline — no API call needed here
...
def run_agent(fac, api_key, rt):
    ...
    # Trimmed system prompt — fewer tokens, same intent
    system = """You are a data center efficiency consultant agent.
Call tools in this exact order:
1. analyze_cooling_efficiency + analyze_power_losses (together)
2. benchmark_against_hyperscalers
3. estimate_improvement_roi (top 1 improvement only)
4. generate_improvement_roadmap
5. Write final report — no more tools.

Final report sections:
🔍 Executive Summary (2-3 sentences)
📊 Key Findings (bullets, use tool numbers)
🏆 Top 3 Actions (with PUE delta + $ savings)
⚡ Quick Wins
🗺️ 18-Month Roadmap
📌 vs Google & Meta"""

    # Compact user message — fewer tokens
    user_msg = (f"Data center: IT={fac['it']:.0f}kW, Cooling={fac['cool']:.0f}kW "
                f"({fac['cool']/fac['it']*100:.0f}%IT), Losses={fac['loss']:.0f}kW "
                f"({fac['loss']/fac['it']*100:.0f}%IT), Lighting={fac['light']:.0f}kW, "
                f"PUE={fac['pue']:.3f}, Total={fac['total']:.0f}kW, "
                f"Rate=${rt}/kWh. Run full analysis.")

    messages  = [{"role": "user", "content": user_msg}]
    tool_log  = []
    max_iters = 10

    def api_call_with_retry(msgs):
        """Call the API with up to 3 retries on 429, with exponential backoff."""
        for attempt in range(3):
            try:
                return client.messages.create(
                    model="claude-sonnet-4-20250514",
                    max_tokens=2048,          # halved from 4096
                    system=system,
                    tools=AGENT_TOOLS,
                    messages=msgs,
                )
    ...
    for iteration in range(max_iters):
        # Pause between iterations to spread token usage across time window
        if iteration > 0:
            time.sleep(4)

        resp = api_call_with_retry(messages)
...
if not api:
    ...
    if not st.session_state.adone:
        ...
        try:
            ...
            status.empty()
            st.warning("⏳ Rate limit reached — the agent retried but the API is still busy. "
                       "Wait 60 seconds and click **Generate the Report** again. "
                       "This happens on free-tier API keys (30k tokens/min limit).")
